...
```
Добавим столбец "Группа"

```{r add_column_new, echo=FALSE}
data1 <- tibble(group = rep(c("Группа 1", "Группа 2"), 50))
data <- data %>% bind_cols(data1)

```

Пример небольшого статистического отчета по категориальным переменным - приводим их к среднему.
```{r stat, echo=FALSE}
data %>%
